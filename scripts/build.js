...
async function main() {
  ...
  // Web export: lets desktop browsers open the prototype directly at the
  // deployment URL (no Expo Go needed). Runs its own Metro, so the Go
  // packager must be stopped first.
  ...
  if (webResult.status !== 0) {
    exitWithError("Web export failed with exit code " + webResult.status);
  }
